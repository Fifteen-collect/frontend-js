...
const createDefaultMatrix = (size: number, method: Types.Method, theme: Types.Theme): Block[][] => {
  const matrix: Block[][] = [];
  ...
  for (let i = 0, count = 1; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      matrix[i][j] = new Block(Types.ColorScheme[theme][method][size][i][j], count, i, j);
...
const isBlockCanMoveDown = (matrix: Block[][], y: number, x: number): boolean => {
...
const isBlockCanMoveLeft = (matrix: Block[][], y: number, x: number): boolean => {
...
const isBlockCanMoveRight = (matrix: Block[][], y: number, x: number): boolean => {
...
const isBlockCanMoveUp = (matrix: Block[][], y: number, x: number): boolean => {
...
const isBlockEmpty = (matrix: Block[][], y: number, x: number): boolean => matrix[y][x].Value === 0;
...
const isMatrixSolved = (matrix: Block[][]): boolean => {
  let counterValue = 0;
